...
{
	_name = name;
	_materialColour = Vector4(1.0f, 0.5f, 0.5f, 1.0f);
}
...
void CubeNode::BuildTexture(const wchar_t* FileName)
{
	...
	ThrowIfFailed(CreateWICTextureFromFile(_device.Get(), _deviceContext.Get(), FileName, nullptr, 	_texture.GetAddressOf()	));
}
