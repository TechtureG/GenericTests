...
// match the format of the vertex used and the format of the input vertex in the shader
...
DirectXApp::DirectXApp() : Framework(800, 600), _indexCountCone{ 0 }, _indexCountTeapot{ 0 }
{
...
bool DirectXApp::Initialise()
{
	...
	ComputeTeapot(_vertices, _indices, 1);
	BuildVertexNormals(_vertices, _indices);
	BuildGeometryBuffers(_vertices, _indices, _vertexBufferTeapot, _indexBufferTeapot);
	_indexCountTeapot = static_cast<unsigned int>(_indices.size());
	ComputeCone(_vertices, _indices, 2, 4, 50);
	//Examples of how other objects could be created
	// ComputeSphere(_vertices, _indices, 4, 50);
	// const Vector3 size(2, 2, 2);
	// ComputeBox(_vertices, _indices, size);
	// ComputeCylinder(_vertices, _indices, 4, 2, 40);
	BuildVertexNormals(_vertices, _indices);
	BuildGeometryBuffers(_vertices, _indices, _vertexBufferCone, _indexBufferCone);
	_indexCountCone = static_cast<unsigned int>(_indices.size());
	BuildShaders();
	BuildVertexLayout();
	BuildConstantBuffer();
...
{
	_worldTransformationTeapot = Matrix::CreateTranslation(Vector3(2, 0, 0)) * Matrix::CreateRotationY(_rotationAngle * XM_PI / 180.0f);
	_worldTransformationCone = Matrix::CreateTranslation(Vector3(0, 0, 3)) * Matrix::CreateRotationX(_rotationAngle * XM_PI / 180.0f);
...
void DirectXApp::Render()
{
	...
	Matrix completeTransformation = _worldTransformationTeapot * _viewTransformation * _projectionTransformation;

	CBuffer constantBuffer;
	constantBuffer.WorldViewProjection = completeTransformation;
	constantBuffer.World = _worldTransformationTeapot;
	constantBuffer.MaterialColour = Vector4(1.0f, 0.0f, 1.0f, 1.0f);
	...
	_deviceContext->IASetVertexBuffers(0, 1, _vertexBufferTeapot.GetAddressOf(), &stride, &offset);
	_deviceContext->IASetIndexBuffer(_indexBufferTeapot.Get(), DXGI_FORMAT_R32_UINT, 0);
	...
	// Now draw the teapot
	_deviceContext->DrawIndexed(_indexCountTeapot, 0, 0);

	// Now draw the cone
	completeTransformation = _worldTransformationCone * _viewTransformation * _projectionTransformation;
	constantBuffer.WorldViewProjection = completeTransformation;
	constantBuffer.World = _worldTransformationCone;
	constantBuffer.MaterialColour = Vector4(1.0f, 1.0f, 0.0f, 1.0f);
	_deviceContext->UpdateSubresource(_constantBuffer.Get(), 0, 0, &constantBuffer, 0, 0);

	// Set the vertex buffer and index buffer we are going to use
	_deviceContext->IASetVertexBuffers(0, 1, _vertexBufferCone.GetAddressOf(), &stride, &offset);
	_deviceContext->IASetIndexBuffer(_indexBufferCone.Get(), DXGI_FORMAT_R32_UINT, 0);

	_deviceContext->DrawIndexed(_indexCountCone, 0, 0);
...
void DirectXApp::BuildGeometryBuffers(vector<ObjectVertexStruct>& vertices, vector<UINT>& indices, ComPtr<ID3D11Buffer>& vertexBuffer, ComPtr<ID3D11Buffer>& indexBuffer)
{
	// buffer should be
	D3D11_BUFFER_DESC vertexBufferDescriptor = { 0 };
	vertexBufferDescriptor.Usage = D3D11_USAGE_IMMUTABLE;
	vertexBufferDescriptor.ByteWidth = sizeof(ObjectVertexStruct) * static_cast<UINT>(vertices.size());
	...
	ThrowIfFailed(_device->CreateBuffer(&vertexBufferDescriptor, &vertexInitialisationData, vertexBuffer.GetAddressOf()));
	...
	ThrowIfFailed(_device->CreateBuffer(&indexBufferDescriptor, &indexInitialisationData, indexBuffer.GetAddressOf()));
}
...
void DirectXApp::BuildVertexNormals(vector<ObjectVertexStruct>& vertices, vector<UINT>& indices)
{
	// Calculate vertex normals
	vector<int> vertexContributingCount(vertices.size(), 0);
