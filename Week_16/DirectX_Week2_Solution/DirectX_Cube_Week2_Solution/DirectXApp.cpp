...
void DirectXApp::Update()
{
	...
	_worldTransformationPyramid = Matrix::CreateTranslation(Vector3(2.5f, 0.0f, 2.5f)) * Matrix::CreateRotationY(_rotationAngle * XM_PI / 180.0f);
...
void DirectXApp::Render()
{
	...
	completeTransformation = _worldTransformationPyramid * _viewTransformation * _projectionTransformation;
	constantBuffer.WorldViewProjection = completeTransformation;
	_deviceContext->UpdateSubresource(_constantBuffer.Get(), 0, 0, &constantBuffer, 0, 0);
	_deviceContext->IASetVertexBuffers(0, 1, _vertexBufferPyramid.GetAddressOf(), &stride, &offset);
	_deviceContext->IASetIndexBuffer(_indexBufferPyramid.Get(), DXGI_FORMAT_R32_UINT, 0);
...
void DirectXApp::BuildGeometryBuffers()
{
	...
	// 
	// Build the vertex and index buffers for the cube
	...
	// Build the vertex and index buffers for the pyramid
	// 
	vertexBufferDescriptor.ByteWidth = sizeof(Vertex) * ARRAYSIZE(pyramidVertices);

	// Now set up a structure that tells DirectX where to get the
	// data for the vertices from
	vertexInitialisationData.pSysMem = &pyramidVertices;

	// and create the vertex buffer
	ThrowIfFailed(_device->CreateBuffer(&vertexBufferDescriptor, &vertexInitialisationData, _vertexBufferPyramid.GetAddressOf()));

	// Setup the structure that specifies how big the index 
	// buffer should be
	indexBufferDescriptor.ByteWidth = sizeof(UINT) * ARRAYSIZE(pyramidIndices);

	// Now set up a structure that tells DirectX where to get the
	// data for the indices from
	indexInitialisationData.pSysMem = &pyramidIndices;

	// and create the index buffer
	ThrowIfFailed(_device->CreateBuffer(&indexBufferDescriptor, &indexInitialisationData, _indexBufferPyramid.GetAddressOf()));
