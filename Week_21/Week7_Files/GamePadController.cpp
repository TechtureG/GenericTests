...
{
	_firstTime = true;
	// We calculate the squares of the dead zone values to avoid having to calculate
	// square roots later
...
void GamePadController::AreControllersEnabled(bool controllerEnabled[4])
{
	DWORD result;
	XINPUT_STATE state;
	ZeroMemory(&state, sizeof(XINPUT_STATE));

	// Test to see if each controller between 0 and 3 is connected
	for (DWORD controller = 0; controller < 4; controller++)
	{
		result = XInputGetState(controller, &state);
		controllerEnabled[controller] = (result == ERROR_SUCCESS);
	}
}
